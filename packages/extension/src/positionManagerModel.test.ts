...
describe('position manager model', () => {
  ...
  it('validates stock positions and permits an empty replacement', () => {
    ...
    expect(parseStockPositionSaveMessage({ command: 'saveStockPositions', positions: [{
      code: 'sh600000', quantity: 100, costPrice: 10, todayTradePrice: 11, soldOut: false,
    }] }, allowed)).toEqual([{
      code: 'sh600000', quantity: 100, costPrice: 10, todayTradePrice: 11, soldOut: false,
    }]);
    expect(parseStockPositionSaveMessage({ command: 'saveStockPositions', positions: [] }, allowed)).toEqual([]);
    expect(() => parseStockPositionSaveMessage({ command: 'saveStockPositions', positions: [{
  ...
  it('rejects partial, duplicate, and non-positive fund positions', () => {
    ...
    expect(parseFundPositionSaveMessage({ command: 'saveFundPositions', positions: [{

...
export function PositionManagerPage({ kind, title, items, initial }: {
  ...
  const save = () => {
    ...
    postMessage(saveType, { positions: [...rows.values()] });
  };

  return <PageShell title={title} dirty={dirty} onSave={save}>
    <section className="content">
      ...
      {showErrors && !valid && <p className="form-error" role="alert">请完成标记的必填项，或清空不需要的持仓行。</p>}
      <div className="table-wrap">
        <table><thead><tr><th>资产</th>{kind === 'stock' ? <><th>数量</th><th>成本价</th><th>今日交易价</th><th>已清仓</th></> : <><th>份额</th><th>成本净值</th></>}<th aria-label="操作" /></tr></thead>
          <tbody>{filteredItems.map((item) => <PositionRow key={item.code} kind={kind} item={item} value={rows.get(item.code)} showErrors={showErrors} update={update} clear={() => clear(item.code)} />)}</tbody>
        </table>
...
function PositionRow({ kind, item, value, showErrors, update, clear }: {
  kind: 'stock' | 'fund'; item: PositionItem; value?: Position;
  showErrors: boolean;
  update: (code: string, patch: Partial<Position>) => void; clear: () => void;
}): ReactElement {
  const input = (name: string, current: number | undefined, required = true) => {
    const invalid = showErrors && value !== undefined && (typeof current !== 'number' || !Number.isFinite(current) || (required ? current <= 0 : current < 0));
    return <input name={name} type="number" min="0" step="any" value={current ?? ''} aria-invalid={invalid || undefined} title={invalid ? (required ? '请输入大于 0 的数值' : '请输入不小于 0 的数值') : undefined} onChange={(event) => {
  ...
  return <tr><td className="asset"><strong>{item.name}</strong><small>{item.code}</small></td>
    {kind === 'stock' ? <><td>{input('quantity', stock?.quantity)}</td><td>{input('costPrice', stock?.costPrice)}</td><td>{input('todayTradePrice', stock?.todayTradePrice, false)}</td><td><input type="checkbox" checked={Boolean(stock?.soldOut)} aria-label={`${item.name} 已清仓`} onChange={(event) => update(item.code, { soldOut: event.target.checked, soldOutDate: undefined })} /></td></> : <><td>{input('shares', fund?.shares)}</td><td>{input('costNav', fund?.costNav)}</td></>}
